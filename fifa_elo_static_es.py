"""
FIFA ELO STATIC ESPAÑOL - Base completa en español
=================================================

Genera base FIFA completa con nombres en ESPAÑOL y ratings Elo realistas.
100% offline. Listo para producción en español.
"""
...
def generate_fifa_elo_database_es(output_csv="fifa_teams_elo_es.csv", output_json="fifa_teams_db_es.json"):
    """
    Genera base FIFA completa con nombres en ESPAÑOL y ratings Elo realistas.
    100% offline. Listo para producción en español.
    """
    
    # Formato: (nombre_español, iso_code, elo_rating, confederación, clasificado_wc2026)
    teams_data = [
        # ── CONMEBOL ─────────────────────────────────────────────────────
        ...
        ("Islas Turcas y Caicos", "TCA", 1198.6, "CONCACAF", False),
        ...
        ("República Democrática del Congo", "COD", 1502.8, "CAF", False),
        ...
        ("Esuatini", "SWZ", 1302.3, "CAF", False),
        ("Malaui", "MWI", 1288.7, "CAF", False),
    ...
    # Exportar
    df.to_csv(output_csv, index=False, encoding="utf-8-sig")  # utf-8-sig para Excel
    
    # JSON para carga rápida
    with open(output_json, "w", encoding="utf-8") as f:
        json.dump(df.to_dict(orient="records"), f, indent=2, ensure_ascii=False)
    
    print(f"✅ Base FIFA en ESPAÑOL generada: {len(df)} selecciones")
    print(f"   • CSV: {output_csv}")
    print(f"   • JSON: {output_json}")
    print(f"   • Clasificadas WC2026: {df['wc_qualified'].sum()}")
    print(f"   • Confederaciones: {df['confederation'].nunique()}")
    print(f"   • Elo rango: {df['elo_rating'].min():.0f} - {df['elo_rating'].max():.0f}")
